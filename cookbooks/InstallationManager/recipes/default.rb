...
package_name = "agent.installer.linux.gtk.x86_64_1.8.5000.20160506_1125.zip"
...
"centos" => { "default" => "yum install unzip" }
)
  action :run
end
...
#  command "#{binary_dir}/userinstc -log #{binary_dir}/instman.log -acceptLicense -dataLocation #{node['InstallationManager']['imagentdata_install_dir']}"
  command "#{binary_dir}/installc -acceptLicense -s -log /tmp/IM_install.log.xml -installationDirectory #{im_dir}"
  cwd binary_dir
  action :run
end
